...
3. Создается 2 стека: стек для хранения результата `result` и стек для хранения
...
4. Выражение просматривается слева-направо, при этом возможно 4 случая:
    ...
    2. Встретилась операция, приоритет которой выше, чем приоритет операции,
    ...
    3. Встретилась операция, приоритет которой равен или ниже приоритета операции,
       ...
       хранения операций.
    4. Встретилась операция ")", тогда из стека `opeartionsstack` все элементы перемещаются
...
2. Выражение просматривается слева-направо, при этом возможно 2 случая:
    ...
    2. Встретилась операция, огда из стека `result` изымаются 2 операнда, над
...
Приложение для списка и стека показывало работу с методами, не запрашивая данных у пользователя,
...
Также была использована система управления версиями Git: была создана ветка, в которой выполнялась
